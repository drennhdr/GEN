  ...
  generateRPP(labOrder: LabOrderModel, rppData: RPPModel, patientData: PatientModel, PhysicianSig: string, PatientSig: string) {
    ...
    doc.addImage(img, 'JPEG', 0, 0, 612, 792);
        ...
        if (labOrder.patientSignatureId == -1)
        {
            ...
            doc.text(formatDate(labOrder.specimens[0].collectionDate, 'MM/dd/yyyy', 'en'),480, 358);
        }
        ...
        if (labOrder.datePatientSignature != null){
            doc.text(formatDate(labOrder.datePatientSignature, 'MM/dd/yyyy', 'en'),485, 358);
        }
        ...
        doc.text(formatDate(labOrder.specimens[0].collectionDate, 'MM/dd/yyyy', 'en'), 280, 436);
    ...
    if (labOrder.userSignatureId_Physician == -1)
    {
        ...
        doc.text(formatDate(labOrder.specimens[0].collectionDate, 'MM/dd/yyyy', 'en'),510, 768);
    }
    ...
    if (labOrder.datePhysicianSignature != null){
        ...
        doc.text(formatDate(labOrder.datePhysicianSignature, 'MM/dd/yyyy', 'en'),510, 768);
    }
    return(doc);
