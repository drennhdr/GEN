  ...
  generateToxUrine(labOrder: LabOrderModel, toxData: ToxModel, patientData: PatientModel, PhysicianSig: string, PatientSig: string) {
   ...
   if (labOrder.selfPay){
    ...
    if (labOrder.patientSignatureId == -1)
    {
        ...
        doc.text(formatDate(labOrder.specimens[0].collectionDate, 'MM/dd/yyyy', 'en'),480, 252);
    }
    ...
    if (labOrder.datePatientSignature != null){
        doc.text(formatDate(labOrder.datePatientSignature, 'MM/dd/yyyy', 'en'),480, 252);
    }
    ...
    doc.text(formatDate(labOrder.specimens[0].collectionDate, 'MM/dd/yyyy', 'en'), 298, 400);
    ...
    if (labOrder.userSignatureId_Physician == -1)
    {
        ...
        doc.text(formatDate(labOrder.specimens[0].collectionDate, 'MM/dd/yyyy', 'en'),460, 773);
    }
    else if (labOrder.datePhysicianSignature != null){
        doc.text(labOrder.user_Physician,300, 773);
        doc.text(formatDate(labOrder.datePhysicianSignature, 'MM/dd/yyyy', 'en'),460, 773);
    }
