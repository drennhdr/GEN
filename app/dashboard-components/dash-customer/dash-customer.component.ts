...
export class DashCustomerComponent implements OnInit {
  ...
  searchButtonClicked(labStatus: number){
      ...
      this.labOrderService.search(this.customerId, this.searchLocationId, labStatus, 0, 0, '', '', this.searchLabTypeId, this.startDate, '', 1 )
        .pipe(first())
        .subscribe(
        data => {
          console.log(data);
  ...
  loadLabSummary(){
    ...
    this.labOrderService.summary(this.searchCustomerId, this.searchLocationId, this.userId, 0, this.searchLabTypeId, this.startDate, this.endDate)
          .pipe(first())
          .subscribe(
          data => {
            if (data.valid)
